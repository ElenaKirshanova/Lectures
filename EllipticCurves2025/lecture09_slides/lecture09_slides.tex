...
\begin{frame}{Атака BSGS или $\rho-$методом Полларда}
    %\begin{itemize}
        %\item 
        Алгоритм BSGS нахождения порядка точки из лекции по подсчёту точек можно адаптировать для поиска \structure{DLOG}.
    %\end{itemize} 
    \vspace{1em}
...
\begin{frame}{$\rho$-метод Полларда}
		\structure{Вход}: $R, P \in E(\mathbb{F}_q)$, $r \in \mathbb{N}$, $h: E(\mathbb{F}_q) \rightarrow \{1, \ldots, r\}$. \\
	\structure{Выход:} $\ell$, т.ч. $R = [\ell] P$\\
	\begin{enumerate}
		\item Сгенерировать $r$ случайных чисел~$\alpha_i$, $\beta_i$;
		\item Вычислить $Q_i = [\alpha_i] P$, $T_i = [\beta_i] R$;
		\item Задать случайное блуждание: $f: (P, a, b) \mapsto (P + Q_{h(P)} + T_{h(P)}, a+\alpha_{h(P)}, b +\beta_{h(P)})$;
		\item $(P_1, a_1, b_1) = (P, 1, 0)$;
		\item $(P_2, a_2, b_2) = f(P_1, a_1, b_1)$;
		\item \structure{while} $P_1 \neq P_2$ \structure{do:}
		\item \quad $(P_1, a_1, b_1) = f(P_1, a_1, b_1)$;
		\item \quad $(P_2, a_2, b_2) = f(f(P_2, a_2, b_2))$;
		\item \structure{return} $\ell = (a_2 - a_1) \cdot (b_1 - b_2)^{-1} \bmod \#\left<P\right>$
	\end{enumerate}
\end{frame}
