...
\subtitle{Лекция 11. Схема обмена ключами CSIDH}
...
\begin{frame}[fragile]{Протокол Диффи-Хеллмана на действиях групп}
	...
	\begin{tikzcd}
		...
		a * x &
		\arrow[yshift=-0.7ex]{r}{a * x}
		\arrow[yshift=0.7ex]{l}{b * x}
		& b * x\\
	\end{tikzcd}
	\]
	%\vspace{-0.5em}
...
\begin{frame}[fragile]{Пример. Классическая схема на ЭК}
	...
	\vspace{1em}
	Аналогично: схема Диффи-Хеллмана на $\mathbb{F}_p^\times$.
\end{frame}
...
\begin{frame}{Кольца эндоморфизмов эллиптических кривых}
	\structure{Кольцо эндоморфизмов}~$\operatorname{End}(E)$ эллиптической кривой~$E$ над конечным полем~$\mathbb{F}_q$ изоморфно\footnote{\scriptsize Deuring M. Die Typen der Multiplikatorenringe elliptischer Funktionenkörper. 1941}:
	\begin{itemize}
		\item порядку в квадратичном мнимом поле\\(\textcolor{struct-color}{обычные кривые})
		\item максимальному порядку в алгебре кватернионов (\textcolor{struct-color}{суперсингулярные кривые})
	\end{itemize}
	
	\vspace{0.5em}
	\structure{Порядок} -- конечно порожденное над~$\mathbb{Z}$ подкольцо (кольца целых в первом случае или алгебры кватернионов во втором).
...
\begin{frame}{Соответствие Дойринга в явном виде}
	Пусть $\mathfrak{a}$ -- идеал порядка~$\mathcal{O}$, который изоморфен $\operatorname{End}(E)$ или его подкольцу. Определим \structure{$\mathfrak{a}$-кручение} как
	\[
...
\begin{frame}{CSIDH}
	...
	\begin{itemize}
		...
		\item $m$ -- наименьшее положительное целое:\\$2m+1 \geq \sqrt[n]{\# \operatorname{Cl}(\mathbb{Z}[\pi_p])}$.
	\end{itemize}
\end{frame}
