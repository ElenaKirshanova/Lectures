...
\begin{frame}{Представление изогений}
	\begin{itemize}
		\item $\phi_{chl}$, $\phi_{rsp}$ -- открыты
		\item от представления зависит размер подписи и скорость проверки
		%\item основная техническая сложность схемы 
		\item представление изогений \structure{не должно} давать возможность вычислять эндоморфизмы:
		\begin{itemize}
			\item по одному эндоморфизму кривой $E$ можно вычислить $End(E)$ за полиномиальное время
		\end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}{SQISign. Оригинальная схема}
	\begin{itemize}
		\item изогении представляются в виде сжатых ядер цепочек изогений малой степени
		\item $\phi_{rsp}$ вычисляется с помощью KLPT-алгоритма
	\end{itemize}
\end{frame}

\begin{frame}{SQISignHD}
	\begin{itemize}
		\item конструктивное применение атаки Кастрика-Декру
		\item изогении представляются в компактном виде $(d, E_{aux}, \phi(P), \phi(Q))$
		%\item $\phi_{rsp}$ вычисляется с помощью KLPT-алгоритма
		\item проверке подписи выполняется Кастрика-Декру
	\end{itemize}
\end{frame}

\begin{frame}{SQIPrime}
	\begin{itemize}
		\item вместо композиций изогений малых степеней используются изогении большой степени и новый алгоритм для их вычисления \href{https://eprint.iacr.org/2023/1766}{Clapoti}
	\end{itemize}
\end{frame}
	...
	\begin{table}[h]
		\begin{center}
			\begin{tabular}{|c|c|c|c|c|}
				...
				\hline
				\href{https://eprint.iacr.org/2020/1240.pdf}{SQISign}
				%SQISign https://eprint.iacr.org/2020/1240.pdf
				 & NIST-1 & 64 & 16 & 204 \\
				\hline
				\href{https://ia.cr/2023/436}{SQISignHD}
				%SQISignHD https://ia.cr/2023/436
				& NIST-1 & 64 & 16 & 109 \\
				\hline
				\href{https://eprint.iacr.org/2024/760.pdf}{SQISign2D-West}
				%SQISign2D-West %https://eprint.iacr.org/2024/760.pdf
				&  NIST-1 & 65 & 353 & 148 \\
				%\hline
				%\href{https://ia.cr/2024/771}{SQISign2D-East}
				%SQISign2D-East %https://ia.cr/2024/771
				%& & & & \\
				\hline
				\href{https://ia.cr/2024/773}{SQIPrime}
...
\begin{frame}{Скорость работы}
	\includegraphics[scale=0.15]{ecc2024-sqisign-perf}
	\begin{itemize}
		\item Источник: Benjamin Wesolowski, ECC2024
	\end{itemize}
\end{frame}
