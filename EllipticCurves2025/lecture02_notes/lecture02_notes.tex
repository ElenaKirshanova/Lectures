...
\begin{document}	
	...
	\begin{equation*}
		...
		\end{split}
	\end{equation*}
	Может получиться так, что точки пересечения нет, как на Рис.~\ref{fig:group:law:inf}. В этом случае полагаем~$P_3 = \mathcal{O}$.
	\begin{figure}[h!]
		\centering
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\For{$j = \ell - 1$ \textbf{to} $0$ \textbf{by} $-1$}
...
\input{../images/fig_dh}
\end{center}
В схеме каждый абонент выбирает свой секретный ключ -- число $a$ (или~$b$), а затем пересылает по открытому каналу значение~$[a]P$ (или~$[b]P$), которое представляют собой открытый ключ. Затем оба абонента по открытым ключам и своему секретному ключу вычисляют общий секретный ключ. Безопасность схемы строится на том факте, что задача вычисления числа $a$ зная только генератор группы~$P$ и точку~$[a]P$, является сложной задачей. Данная задача называется \structure{задачей вычисления дискретного логарифма (DLOG)}. На сегодняшний день не известно классического (не квантового) алгоритма, который в общем случае решал бы данную задачу за полиномиальное время.

Заметим, что на практике схемы шифрования с открытым ключом не используются для шифрования большого количества информации из-за медленной (по сравнению с симметричными шифрами) скорости работы. Вместо этого используется гибридный подход -- сначала вырабатывается общий секретный ключ с использованием протокола обмена ключами. А затем данный ключ применяется для шифрования данных с помощью симметричного шифра.

\section{Атака ``человек посередине''}
Протокол Диффи-Хелмана уязвим к атаке ''человек посередине'', в которой имеется третья сторона, которая имеет возможность перехватывать и подменять открытые сообщения абонентов. В качестве такой третьей стороны может выступать провайдер интернета или владелец точки доступа. Схема атаки представлена на следующем рисунке.

\begin{center}
	\input{../images/fig_mitm}
\end{center}
Третья сторона вырабатывает с каждым из абонентов свой секретный ключ и с помощью секретных ключей перешифровывает переписку, попутно читая её. 
