...
\begin{document}	
	{\noindent
		...
		Преподаватель {С. Новоселов}   \hfill{Осень 2026\\}
	\hrule
	\begin{center}
		{\LARGE\textbf{
				Лабораторная работа № 1 \\[5pt]
		}} 
			Опубликована \textbf{01.09.2026} \\[5pt] 
			Дедлайн \textbf{15.09.2026}
...
\section*{Задание 2}
Разработать программу в Sage, проверяющую для кривой над полем~$\mathbb{F}_{p^6}$ возможность изоморфного преобразования в форму Шолтена (Scholten): $y^2 = a x^3 + b x^2 + b^{p^3} x + a^{p^3}$. Если такое преобразование существует, вывести его в явном виде в виде коэффициентов $(u, r, s, t)$, задающих изоморфизм.

Кривые над полем~$\mathbb{F}_{p^6}$ используются в классической криптографии на билинейных отображениях в доказательствах с нулевым разглашением (основное приложение -- криптовалюты). Кривые в форме Шолтена уязвимы к атакам с помощью накрытий~\cite{joux2012cover} и таким образом мы проверяем уязвимость кривой к атаке.

Пример кривой, которая приводится в форму Шолтена:
\begin{itemize}
	\item $E: y^2 = x^3 + (6 \alpha^5+4 \alpha^4+9 \alpha^3+10 \alpha^2+4) x + (5 \alpha^5+8 \alpha^4+\alpha^2+10 \alpha+7)$
	\item $\mathbb{F}_{11^6} \simeq \mathbb{F}_{11}[\alpha] / \left< \alpha^6 + 3 \alpha^4 + 4 \alpha^3 + 6 \alpha^2 + 7 \alpha + 2 \right>$
\end{itemize}
...
\begin{itemize}
	...
	\item Студент должен понимать, что он написал, зачем, а также ответить на теоретические вопросы по лекции.
\end{itemize}
