...
\structure{Проверка}
\begin{enumerate}
	\item $e_r(\text{\structure{\faPaw}}, P_2) == e_r(H(\text{\structure{\footnotesize\faEnvelope}}), Q)$?
\end{enumerate}
\vspace{1em}
\ProofBegin
$e_r(\text{\structure{\faPaw}}, P_2) = e_r(R, P_2)^a = e_r(H(\text{\structure{\footnotesize\faEnvelope}}), [a] P_2) = e_r(H(\text{\structure{\footnotesize\faEnvelope}}), Q)$
\ProofEnd
...
\begin{itemize}
	\item В Ethereum каждые 12 секунд формируется новый блок из транзакций (напр., перевод средств)
	\item Блок проверяется и подписывается тысячами валидаторов
	\item Хранение тысяч подписей валидаторов $\text{\structure{\faPaw}}_1, \ldots, \text{\structure{\faPaw}}_m$ заняло бы много места в блокчейне
	\item Поэтому подписи объединяются в одну (96-байт):
	\[
	\text{\structure{\faPaw}} = \text{\structure{\faPaw}}_1 + \ldots +  \text{\structure{\faPaw}}_m
	\]
