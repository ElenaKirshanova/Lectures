% !TeX document-id = {52401aef-1b0e-46ee-a454-1c7cf4bec39f}
% !TeX program = xelatex
...
\begin{document}
	...
	\begin{frame}{Порядок группы~$E(\mathbb{F}_q)$}
		\begin{center}
			\begin{tcolorbox}[enhanced,hbox,colback=box-blue-color!15,colframe=box-blue-color,title=Теорема Хассе,center title]
				\begin{varwidth}{\textwidth}
		...
		\end{center}
		\begin{itemize}
			\item определение точного значения:\\алгоритмы подсчёта точек
			\item самый быстрый известный: SEA (Schoof-Elkies-Atkin)
			\item сложность (эвристическая): $\mathcal{O}(\log^4{q})$ 
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Безопасность протокола I}
		 Безопасность основана на (как минимум) сложности нахождения задач \structure{DLOG} и \structure{ECDHP}.
		 
		 \vspace{1em}
		\begin{itemize}
			\item \structure{DLOG} (дискретный логарифм):
			\[(P, [a]P) \mapsto a\]
			\item \structure{ECDHP} (Вычислительная задача Диффи–Хеллмана):
			\[(P, [a]P, [b]P) \mapsto [a b] P\]
		\end{itemize}
	\end{frame}
	
	\begin{frame}{Безопасность протокола II}
		Взаимосвязь между задачами:
		\begin{itemize}
			\item \structure{DLOG} \structure{$\implies$} \structure{ECDHP}
			\item \structure{ECDHP} \structure{$\implies$} \structure{DLOG}, открытая проблема
		\end{itemize}
	\end{frame}
	
	\begin{frame}{Безопасность протокола III}
		Наилучший известный алгоритм решения задачи \structure{DLOG} в общем случае:
		\begin{itemize}
			\item метод Полларда, сложность $\mathcal{O}(\sqrt{q})$
			\item есть большое число частных случаев, где задача решается проще (см. лекцию по выбору кривой)
			\item на квантовом компьютере задача \structure{DLOG} решается за~\structure{полиномиальное время}
		\end{itemize}
	\end{frame}

	\begin{frame}{Атаки по побочным каналам}
		В алгоритме быстрого умножения время работы и энергопотребление выдают значение~$k$ (= ключ в криптографических протоколах).
		
		\vspace{1em}
		
		В приложениях используются вариации алгоритма с одним и тем же временем выполнения наы разных входах.
	\end{frame}
