...
\begin{document}	
	{\noindent
		...
		Преподаватель {С. Новоселов}   \hfill{Осень 2020\\}
	\hrule
	\begin{center}
		{\LARGE\textbf{
				Лабораторная работа № 2 \\[5pt]
		}} 
			Опубликована \textbf{18.09.2020} \\[5pt] 
			Дедлайн \textbf{02.10.2020 (16:50)}
	...
	Разработать программу в системе компьютерной алгебры Sage (в одной на выбор), реализующую следующие функции:
...
\begin{itemize}
     ...
     \item Лабораторную следует выполнять модификацией файла с тестами (TP2\_tests.sage), заменяя строку "\texttt{\# your code here.}" на код, реализующий функцию.
     \item Функции должны работать на всех примерах, что проверяется запуском команды:
      \\\texttt{sage -t TP2\_tests.sage}\\
\end{itemize}
\end{document}