...
\begin{frame}{Мотивация}
    ...
    \begin{enumerate}
        ...
        \end{itemize}
    \end{enumerate}
    $CM$-метод даёт более оптимальные кривые, но при этом существенно сложнее.
\end{frame}
...
\structure{Действие $\varphi_q$} на кривую $E/\mathbb{F}_q$:
\[
...
\structure{Выход:} $k = ord(P)$.
\begin{enumerate}
    \item $Q = (q+1) P$.
    \item Выбрать $m > \sqrt[4]{q}$, вычислить и сохранить в списке $L$ все пары $(j, j P)$ для $j = 0, ..., m$. \hfill{\structure{\textit{(baby steps)}}}
    \item \label{bsgs:order:step:giant}
    Вычислять точки $Q + k (2m P)$ для $k = -m, -(m-1), ..., m$ пока в $L$ не найдётся точка $\pm j P$ т.ч. $Q + k(2m P) = \pm j P$ \hfill{\structure{\textit{(giant steps)}}}
    \item Имеем $(q + 1 + 2 m k \mp j) P = \mathcal{O}$.\\$M = q + 1 + 2 m k \mp j$.
    \item \label{bsgs:order:step:factor} Найдём $p_1, ..., p_r$ -- различные простые делители $M$.
...
\structure{Шаг~1.} (быстрое умножение) $O(\log{q})$ сложений на кривой \structure{$\implies$} $O(\operatorname{polylog}{q})$ бит. операций
