...
Точнее: $p-1 = \prod p_i^{e_i},\ p_i \leq B_1, p_i^{e_i} \leq B_2$ \\(такие $p-1$ называются \structure{$B_1$-гладкими})

\structure{Идея метода:}
\begin{itemize}
    \item (теорема Ферма): $\forall a \in \mathbb{Z}_N^\times$ и $\forall k$ -- кратного $p-1$:
    \[
...
\begin{enumerate}
    ...
    \item \structure{for} всех простых $p_1 < B_1$ и $e_i: p_i^{e_i} < {B_2}$:\\
    \quad $
