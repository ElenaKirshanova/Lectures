...
\begin{document}	
	...
	\begin{center}
		{\LARGE\textbf{
			...
			Опубликована \textbf{20.09.2019} \\[5pt] 
			Дэдлайн \textbf{04.10.2019 (16:50)}
	...
	\begin{enumerate}
		\item \texttt{Sum($\mathtt{a, b, q,  x1, y1, x2, y2}$)}, где $\mathtt{a, b}$ -- коэффициенты эллиптической кривой $E$, заданной над полем $\FF_q$, где $q$ - простое, $\neq 2,3$, $P_1 = \mathtt{(x1, y1)}, P_2 = \mathtt{(x2, y2)}$-- точки на $E$ ($y_i  = \text{infinity}$ для $P_i = \mathcal{O}$). Функция возвращает координаты $P_3 = \mathtt{(x3, y3)}= P_1 + P_2$. Если $P_1$ или $P_2$ не лежат в $E$, функция возвращает ошибку.
